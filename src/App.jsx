...
const App = () => {
  ...
  const {
    ...
    resetGeneration,
    setError,
    setDbEndTime, // ✅ Added so you can call it later
  } = useImageGeneration();
  ...
  useEffect(() => {
    if (!loading && generationId && !dbEndTime) {
      const fetchEndTime = async () => {
        ...
        if (data?.created_at) {
          setDbEndTime(new Date(data.created_at).getTime()); // ✅ Works now
        }
      };

      fetchEndTime();
    }
  }, [loading, generationId, dbEndTime, setDbEndTime]);
                      ...
                      <div className="col-span-1 sm:col-span-2 p-6 flex text-neutral-400 rounded-lg shadow-sm items-center justify-center h-full">
                        {loading ? (
                          ...
                          <div className="w-full flex flex-col items-center gap-2">
                            <i
                              className="ri-error-warning-line text-red-400 text-3xl"
                              aria-hidden="true"
                            ></i>
                            <p
